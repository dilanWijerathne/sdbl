...
class DataController extends Controller
{
    ...
    public function check_applicant_with_current_banking_data(Request $request)
    {
        ...
        Log::info('NIC Check with Core');
        Log::info($request);


        $url = "";
        if (env('APP_LIVE') === "yes") {
            Log::alert('NIC Customer APP L- ' . env('APP_LIVE') . " point -> " .  env('NIC_CUS'));
            $url =  env('NIC_CUS');
        } elseif (env('APP_LIVE') === "no") {
            Log::alert('NIC customer APP L- ' . env('APP_LIVE') . " point -> " . env('NIC_CUS_TEST'));
            $url =   env('NIC_CUS_TEST');
        }
    ...
    public function create_account(Request $request)
    {
        ...
        $gg = "http://10.100.32.202:7802/account_creationa/v1/accountCreationA";
        $t = 'http://10.100.32.72:7801/account_creation/v1/accountCreation';
        $responseC = Http::post($gg, [
    ...
    public function check_sdb_account(Request $request)
    {
        ...
        $responseB = Http::post('http://10.100.32.191:7080/accbala/v1/AccountBalanceInformationA?AccountNumber=00001089327&AccountType=S', [
