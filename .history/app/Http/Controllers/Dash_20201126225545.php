...
class Dash extends Controller
{
    ...
    public function create_account($para)
    {
        ...
        $url = "";
        if (env('APP_LIVE') === "yes") {
            Log::alert('ACC APP L- ' . env('APP_LIVE'));
            $url = "http://10.100.32.202:7802/account_creationa/v1/accountCreationA";
        } elseif (env('APP_LIVE') === "no") {
            Log::alert('ACC APP L- ' . env('APP_LIVE'));
            $url = 'http://10.100.32.72:7801/account_creation/v1/accountCreation';
        }
