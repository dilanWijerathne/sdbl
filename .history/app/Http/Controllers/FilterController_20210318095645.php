        ...
        if ($user === 0 | $user === "0") {
            ...
            if ($request->current_branch_search === "0") {
                ...
                if ($request->app_status === '10' && $product === "all") {
                    ...
                    $models = DB::table('applicant')
                        ->select('ref', 'title',  'full_name',  'nic', 'primary_mobile_number', 'updated_at', 'created_at', 'signed')
                        ...
                        ->get()
                        ->map(function ($item) {
                            return [$item->ref, $item->title,  $item->full_name,  $item->nic, $item->primary_mobile_number, $item->updated_at, $item->created_at, $item->signed];
                        })->toArray();
            ...
            /*else {
                    ...
                    $models = DB::table('applicant')
                        ...
                        // ->orWhere('primary_mobile_number', 'LIKE', '%' . $request->search . '%')
                        // ->orWhere('full_name', 'LIKE', '%' . $request->search . '%')
                        ->orderBy('updated_at', 'created_at', 'desc')
                        ->limit($request->end)->offset($request->start - 1)
                        ->get()
                        ->map(function ($item) {
                            return [$item->ref, $item->title,  $item->full_name,  $item->nic, $item->primary_mobile_number, $item->updated_at, $item->signed];
                        })->toArray();
