        ...
        if ($user === 0 | $user === "0") {
            ...
            if ($request->app_status != "10" && $product === "savings") {
                ...
                $models = DB::table('applicant')
                    ...
                    ->where([['done', (int)$request->app_status], ['branch', $user], ['applicant_going_to_open', '!=', 'Fixed Deposits'], ['nic', 'LIKE', $request->search . '%']])
            ...
            if ($request->app_status != "10" && $product === "all") {
                ...
                $models = DB::table('applicant')
                    ...
                    // ->orWhere('primary_mobile_number', 'LIKE', '%' . $request->search . '%')
                    // ->orWhere('full_name', 'LIKE', '%' . $request->search . '%')
                    ->where([['done', (int)$request->app_status], ['branch', $user], ['nic', 'LIKE', $request->search . '%']])
                    ->orderBy('created_at', 'desc')
                    ->limit($request->end)->offset($request->start - 1)
                    ->get()
                    ->map(function ($item) {
